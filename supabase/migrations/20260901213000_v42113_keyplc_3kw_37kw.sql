...
-- Existing rows/prices are preserved. New rows start with blank USD/RMB/MYR prices.
...
begin
  ...
  if not exists (
    ...
    insert into public.ks_products_keyplc
      (model, motor_kw, source_row, rarity, variants, status)
    values
      ('3kW', 3.0, v_next_source, 'common', v_blank_variants, 'active');
  ...
  if not exists (
    ...
    insert into public.ks_products_keyplc
      (model, motor_kw, source_row, rarity, variants, status)
    values
      ('37kW', 37.0, v_next_source, 'common', v_blank_variants, 'active');
  else
    update public.ks_products_keyplc
       set model='37kW',
           motor_kw=37.0,
...
-- Verification
select model, motor_kw, source_row, rarity, status, variants
from public.ks_products_keyplc
where abs(coalesce(motor_kw,0)::numeric - 3.0) < 0.0001
