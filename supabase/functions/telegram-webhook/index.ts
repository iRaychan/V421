...
function guidedProductGroupMeta(groupValue:any){
  ...
  return {group,roleFamily,productLabel,productType,hasCurve:group==='CHC_G2'||group==='ES'};
}
async function guidedUserAvailableProducts(service:any,companyId:string,user:any){
  ...
  const [br,sr,mr]=await Promise.all([
    service.from('ks_oem_brands').select('id,brand_name,brand_key,logo_data,active').eq('company_id',companyId).eq('active',true).limit(1000),
  ...
  const brandNames=new Map<string,string>(),brandLogos=new Map<string,string>();
...
async function guidedSizeSelectedProducts(service:any,companyId:string,products:any[],q:number,h:number){
  ...
  for(const product of products||[]){
    ...
    for(const pole of poles){let rows:any[]=[];try{rows=selectPumpCandidates(group==='ES'?'ES':'CHC',q,h,pole,3)||[]}catch(_){rows=[]}
...
async function keybotFastOpenMatch(service:any,telegramToken:string,companyId:string,chatId:string,senderId:string,session:any,customer:any,match:any,context:any={}){
  ...
  try{
    if(product?.has_curve===true&&(group==='CHC_G2'||(group==='ES'&&[2,4].includes(pole)))){
      ...
      if(group==='ES'){
        ...
        const info=directChcModelInfo(exact?.master_model);if(info)lines.push('Type: Vertical Multistage Inline Pump',Number(info.motor_kw)>0?`Motor: ${inputNumber(info.motor_kw)} kW / ${inputNumber(info.motor_hp)} HP`:null,info.connection?`Connection: ${info.connection}`:null);
      }
...
async function quotePumpForCustomer(service:any,customerId:string,item:any,keySuiteUserEmail:string,pricingOverride:any={}){
  ...
  if(family==='CHC'){
    ...
    let productRes=await service.from('ks_products_chc').select('*').ilike('model',model).maybeSingle();if(productRes.error||!productRes.data)throw new Error(`No CHC price-list record was found for ${item?.model||model}.`);let p:any=productRes.data;productId=String(p.id||'');
    ...
    if(material!=='CHC'&&!candidates.some((x:any)=>Number(x.sourcePrice)>0)){
      ...
      const exactRes=exactSku?await service.from('ks_products_chc').select('*').ilike('model',exactSku).maybeSingle():null;
...
function exactChcRatedPoint(model:any){
  const db:any=(globalThis as any).KeySuiteCHCData,core:any=(globalThis as any).KeySuiteCHCCore,wanted=String(model||'').replace(/^VMS\b/i,'CHC').replace(/\s+/g,' ').trim().toUpperCase();
  const row=(db?.models||[]).find((m:any)=>String(m.model||'').toUpperCase()===wanted);if(!row||!core)return null;
  // V4.18.07: the CHC family number is the nominal rated flow.
  // Example: CHC 10-100 is rated at Q=10.0 m³/hr. BEP remains a separate
  // hydraulic property and must not move the Rated duty marker to 9.8 m³/hr.
...
async function guidedSendExactRatedCurve(service:any,telegramToken:string,companyId:string,chatId:string,senderId:string,session:any,customer:any,product:any,exact:any,pole:any=0){
  const group=String(product?.price_group||'').toUpperCase(),family=group==='ES'?'ES':'CHC',esPole=family==='ES'?Number(pole||exact?.pole||0):0;
  const rated:any=family==='ES'?exactEsRatedPoint(exact?.master_model,esPole):exactChcRatedPoint(exact?.master_model);
...
async function guidedCompletePump(service:any,telegramToken:string,companyId:string,chatId:string,senderId:string,session:any,customer:any,user:any,product:any,request:any,action:'curve'|'price'|'select'){
  const c=sessionContext(session),merged=mergeQuoteRequest(c.guided_pending_request,request),q=Number(merged.flow_m3h||0),h=Number(merged.head_m||0),group=String(product?.price_group||'').toUpperCase(),family=group==='ES'?'ES':'CHC';
